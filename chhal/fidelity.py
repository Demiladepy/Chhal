...
# zero. Measured legit-vs-legit at n=m=500 from two DISJOINT samples, 100 reps
# (scripts/audit/ks_null_floor.py), the floor spans nearly fourteen-fold across the
# feature space, from `is_cross_border` at 0.0040 to `amount` at 0.0550, mean 0.0341. So
# `< 0.1` meant "within 1.8x of pure noise" on one row of the table and "within 25x" on
# another, in the same table, and the rows were being read as if they said the same thing.
#
...
KS_NULL_FLOOR = {
    "amount": 0.0550,
    "time_since_last_txn_min": 0.0467,
    "hour": 0.0449,
    "day_of_week": 0.0422,
    "amount_to_avg_ratio": 0.0397,
    "velocity_24h": 0.0329,
    "channel_code": 0.0280,
    "is_new_beneficiary": 0.0249,
    "velocity_1h": 0.0222,
    "is_cross_border": 0.0040,
}
...
KS_NULL_FLOOR_DEFAULT = 0.0341
# These floors replace an earlier set produced by a biased estimator, which drew the second
# legit sample from WITHIN the reference set (ref = full legit, sample = legit.sample(500)).
# Sharing rows between the two ECDFs biases the two-sample KS DOWNWARD, so the old floors
# were too low and every degradation_ratio built on them was correspondingly too high. The
# bias was NOT uniform across features. Measured against the disjoint estimator it ranged
# from 0.97x on `amount_to_avg_ratio` to 2.15x on `velocity_24h`, so it moved the ordering
# and not merely the scale: `threshold_hugging` and `autopay_mandate` swap places once the
# floors are correct. The values above are the disjoint measurement on real IEEE-CIS data.
...
# rule reads "within twice the expected noise", and measured on two DISJOINT 500-row legit
# draws (seed 7, full IEEE-CIS legit population) it passes 25 of 26 features at a mean
# ratio of 0.84, against 5.68 for a 500-row draw of real IEEE-CIS fraud over the same
# features. The single failure is single-draw fluctuation, not a defect, and the ratio
# column is there precisely so a reader can see that rather than trust the flag.
LEGIT_RATIO = 2.0
...
def ks_null_floor(feature: str, n: int, m: int) -> float:
    ...
    The floor is not a constant across sample size either. It fell from 0.0341 at
    n=m=500 to 0.0111 at n=5,100 in the same measurement. So the measured value is
